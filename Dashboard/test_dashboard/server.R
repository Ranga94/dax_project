...
server <- function(input, output){
  ...
  from_date <- '2017-12-02 00:00:00'
  to_date <-'2017-12-31 00:00:00'
  ...
  output$popular_treemap<-renderPlot({
    sql <- 'SELECT * FROM[igenie-project:pecten_dataset.twitter_sentiment_popularity];'
    #sql<- paste("SELECT * FROM pecten_dataset.twitter_sentiment_popularity AND date between TIMESTAMP('", from_date, " UTC')",
 ...
 # News
  ...
  sql <- 'SELECT From_date, To_date, constituent,NEWS_TITLE_NewsDim,NEWS_DATE_NewsDim,NEWS_ARTICLE_TXT_NewsDim,categorised_tag,sentiment FROM[igenie-project:pecten_dataset.news_all];'
  ...
  sql <- 'SELECT * FROM[igenie-project:pecten_dataset.analyst_opinions];'
  ...
  sql <- paste("SELECT * FROM[igenie-project:pecten_dataset.summary_box] WHERE From_date = '", from_date ,"';",sep='')
  ...
  output$CRtable  <- renderDataTable({
    sql <- paste("SELECT * FROM [igenie-project:pecten_dataset.cumulative_returns] WHERE To_date= '", to_date ,"';", sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
    retrieved_data<-na.omit(retrieved_data)
  ...
  output$ranking_top  <- renderDataTable({
    sql <- paste("SELECT * FROM[igenie-project:pecten_dataset.Profitability_tag_ranking] WHERE To_date= '", to_date ,"';", sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
    retrieved_data<-na.omit(retrieved_data)
  ...
  output$EPS_table <- renderDataTable({
    sql <- 'SELECT Constituent,Current_EPS,EPS_last_year FROM[igenie-project:pecten_dataset.EPS_t] WHERE STATUS ="active";'
  ...
  output$PER_table <- renderDataTable({
    sql <- 'SELECT Constituent,Current_PER,PER_last_year FROM[igenie-project:pecten_dataset.PER_t] WHERE STATUS ="active";'
  ...
  output$target_price_error <- renderText({
    ...
    sql <- paste('SELECT * FROM[igenie-project:pecten_dataset.target_prices] WHERE constituent ="',constituent,'";',sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
  ...
  output$influencer_price_error <- renderText({
    if (constituent =="Adidas"){constituent = 'adidas'}
    sql <- paste('SELECT * FROM[igenie-project:pecten_dataset.influencer_prices] WHERE constituent ="',constituent,'";',sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
    p<-nrow(retrieved_data)
    p
  })
  ...
  output$tweet_num <-renderPlot({
    constituent = toString(input$constituent)
    if (constituent =="Adidas"){constituent = 'adidas'}
    #sql <- 'SELECT * FROM[igenie-project:pecten_dataset.twitter_sentiment_count_daily];'
    sql <- paste('SELECT * FROM[igenie-project:pecten_dataset.twitter_sentiment_count_daily] WHERE constituent ="',constituent,'";', sep='')
  ...
  output$sentiment_map<-renderPlot({
    constituent = toString(input$constituent)
    if (constituent =="Adidas"){constituent = 'adidas'}
    sql <- paste('SELECT count,avg_sentiment,country_name,constituent,from_date,to_date FROM[igenie-project:pecten_dataset.country_data] WHERE constituent ="',constituent,'";', sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
  ...
  output$recent_tweets_table <- renderDataTable({
    constituent = toString(input$constituent)
    if (constituent =="Adidas"){constituent = 'adidas'}
    sql <- paste('SELECT * FROM[igenie-project:pecten_dataset.twitter_analytics_latest_price_tweets] WHERE constituent="',constituent,'";', sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
    #删去重复text
  ...
  output$news_tag_bar<-renderPlot({
    constituent <-toString(input$constituent)
    if (constituent == 'Adidas'){constituent = "adidas"}
    sql <- paste('SELECT From_date,To_date,Tags,Count FROM[igenie-project:pecten_dataset.news_tag] WHERE constituent="',constituent,'";', sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
  ...
  output$news_sentiment_daily<-renderPlot({
    constituent <-toString(input$constituent)
    if (constituent == 'Adidas'){constituent = "adidas"}
    sql <- paste('SELECT * FROM[igenie-project:pecten_dataset.news_analytics_daily_sentiment] WHERE constituent="',constituent,'";', sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
    retrieved_data$To_date<-strptime(retrieved_data$To_date,format = "%Y-%m-%d")
    retrieved_data$From_date<-strptime(retrieved_data$From_date,format = "%Y-%m-%d")
    #retrieved_data$date <-as.Date(retrieved_data$date)
  ...
  output$topic_sentiment_grid<-renderPlot({
    constituent <-toString(input$constituent)
    sql <- paste('SELECT * FROM[igenie-project:pecten_dataset.news_analytics_topic_sentiment] WHERE constituent="',constituent,'";', sep='')
    retrieved_data <- query_exec(project=project,  sql, billing = project)
  ...
  #sql <- 'SELECT NEWS_DATE_NewsDim,constituent,categorised_tag,NEWS_TITLE_NewsDim, NEWS_ARTICLE_TXT_NewsDim FROM[igenie-project:pecten_dataset.news_analytics_topic_articles];'
  sql<- paste("SELECT From_Date,To_Date, NEWS_DATE_NewsDim,constituent,categorised_tag,News_Title_NewsDim, NEWS_ARTICLE_TXT_NewsDim FROM[igenie-project:pecten_dataset.news_analytics_topic_articles] WHERE From_Date =TIMESTAMP('", from_date, " UTC') AND To_Date = TIMESTAMP('", to_date, " UTC') ;",sep='')
  ...
  ################################# Correlation Page ######################################
  sql <- 'SELECT * FROM[igenie-project:pecten_dataset.all_correlations];'
