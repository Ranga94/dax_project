...
def main(args):
    dax_url = 'http://en.boerse-frankfurt.de/index/pricehistory/DAX/1.1.2003_17.11.2017#History'
    ...
    if args.all:
        ...
        for c in all_constituents.keys():
            extract_historical_data(urljoin(constituent_base_url, all_constituents[c] + '-share/FSE/1.1.2003_17.11.2017#Price_History'), driver, db, constituent=c)
            time.sleep(60)
    else:
        if args.constituent == 'DAX':
            ...
            if args.constituent in all_constituents.keys():
                constituent_url = urljoin(constituent_base_url, all_constituents[args.constituent] + '-share/FSE/1.1.2003_17.11.2017#Price_History') #check const url name
