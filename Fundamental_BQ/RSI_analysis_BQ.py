...
#python RSI_analysis_BQ.py 'mysql+pymysql://igenie_readwrite:igenie@127.0.0.1/dax_project' 'PARAM_FINANCIAL_KEY_COLLECTION' 'igenie-project-key.json' 
...
def RSI_main(args):
    ...
    for constituent in constituent_list:
        ...
        RSI_table = RSI_table.append(pd.DataFrame({'Constituent':constituent,'Constituent_name':constituent_name, 'Constituent_id':constituent_id, 'Current_RSI':round(RSI_current,2),'percentage_days_overbought':round(overbought_pct*100,2),'percentage_days_oversold':round(oversold_pct*100,2),'RSI_bull_score':RSI_score,'Table':'RSI analysis','Date':date,'Status':'active'},index=[0]),ignore_index=True)
...
def update_result(table_store):
    table_store = 'pecten_dataset.RSI_t'
    #import os
...
def get_historical_price(project_name,table_historical,constituent):
    ...
    QUERY ='SELECT * FROM '+ table_historical + ' WHERE Constituent= "'+constituent+'";'
    print QUERY
