...
library(plotly)
#source('/Users/kefei/Documents/Igenie_Consulting/dashboard_final_sample/Homepage_subfunctions.R')
#source('/Users/kefei/Documents/Igenie_Consulting/dashboard_final_sample/Fundamental_subfunctions.R')
#source('/Users/kefei/Documents/Igenie_Consulting/dashboard_final_sample/Analyst_subfunctions.R')
#source('/Users/kefei/Documents/Igenie_Consulting/dashboard_final_sample/Twitter_subfunctions.R')
#source('/Users/kefei/Documents/Igenie_Consulting/dashboard_final_sample/News_subfunctions.R')
#source('/Users/kefei/Documents/Igenie_Consulting/dashboard_final_sample/Correlation_subfunctions.R')
...
######################################  Main content of the dashboard  ##############################################
  ...
  twitter_count <- mongo(collection = 'twitter_sentiment_count_daily2',
                         url = url_mongo,
                         verbose = FALSE, options = ssl_options())
  twitter_df<-twitter_count$find('{}')
  twitter_df$date<-as.Date(twitter_df$date)
  twitter_df<- twitter_df[twitter_df$date>as.Date('2017-10-06'),]
  twitter_df<- twitter_df[twitter_df$date<as.Date('2017-11-16'),]
  ...
  EPS_analysis <- eventReactive(input$reload, {
    db <- mongo(collection = 'fundamental analysis',
                url = url_mongo,
                verbose = FALSE, options = ssl_options())
    retrieved_data <- db$find('{"Table":"EPS analysis","Status":"active"}')
    EPS_table(retrieved_data)
  },ignoreNULL = FALSE)
  output$EPS_table <- renderDataTable(EPS_analysis())
  ...
  PER_analysis <- eventReactive(input$reload, {
    db <- mongo(collection = 'fundamental analysis',
                url = url_mongo,
                verbose = FALSE, options = ssl_options())
    retrieved_data <- db$find('{"Table":"PER analysis","Status":"active"}')
  ...
  country_df<- country_df[country_df$`date of analysis` == as.Date('2017-11-20'),]
  ...
  news_tag_score <- mongo(collection = 'news_tagging_score',
                    url = url_mongo,
                    verbose = FALSE, options = ssl_options())
  tag_score_df<-news_tag_score$find('{}')
  tag_score_df$date<-as.Date(tag_score_df$date)
  tag_score_df<-tag_score_df[tag_score_df$date > as.Date('2017-11-06'),]
  #selected_tag_score_df<-tag_score_df[tag_score_df$date>as.Date('2017-11-06'),]
