...
SELECT
  ...
  constituent_name,
  constituent_id,
  ARRAY_TO_STRING(MONEY, ",") as entity_tags,
  date AS tweet_date,
  ...
  CASE
    ...
    END AS date
FROM
  {0}.twitter_analytics_latest_target_prices
WHERE
  followers_count > 1000 and
...
""".format(common_parameters["BQ_DATASET"],common_parameters["FROM_DATE"].strftime('%Y-%m-%d %H:%M:%S'),
    ...
    try:
        storage_client.insert_bigquery_data(common_parameters["BQ_DATASET"], 'influencer_prices', to_insert)
        print(len(to_insert))
