...
#python sales_analysis_BQ.py 'mysql+pymysql://igenie_readwrite:igenie@127.0.0.1/dax_project' 'PARAM_FINANCIAL_KEY_COLLECTION' 'igenie-project-key.json' 'pecten_dataset_test.sales_t'
...
def sales_calculate(master):
    table= master[['Sales_in_Mio','year']].dropna(thresh=2)
    #print table
    table['Sales_in_Mio']=table['Sales_in_Mio'].str.replace(",","").astype(float)
    table = table.reset_index(drop=True)
    #print float(table['Sales in Mio'].iloc[-1])
    
    if len(table)>0:
        pct_sales_last_year = 100*(float(table['Sales_in_Mio'].iloc[-1])-float(table['Sales_in_Mio'].iloc[-2]))/float(table['Sales_in_Mio'].iloc[-2])
        pct_sales_four_years = 100*(float(table['Sales_in_Mio'].iloc[-1])-float(table['Sales_in_Mio'].iloc[-4]))/float(table['Sales_in_Mio'].iloc[-4])
        ...
        pct_sales_last_year=0
        pct_sales_four_years=0
        table['Sales_in_Mio']=[0]
        score = 0
    return float(pct_sales_last_year), float(pct_sales_four_years), table['Sales_in_Mio'], score
