...
def get_target_prices(args):
    ...
    # Feature PECTEN-9
    backup_table_name = backup_table(args.google_key_path, common_parameters["BQ_DATASET"], 'target_prices')
...
""".format(common_parameters["BQ_DATASET"],common_parameters["FROM_DATE"].strftime('%Y-%m-%d %H:%M:%S'),
    ...
    try:
        before_insert(args.google_key_path,common_parameters["BQ_DATASET"],'target_prices',from_date,to_date,storage_client)
    ...
    try:
        ...
        # Feature PECTEN-9
        if storage_client.insert_bigquery_data(common_parameters["BQ_DATASET"], 'target_prices', to_insert):
        ...
        rollback_object(args.google_key_path, 'table', common_parameters["BQ_DATASET"], None,
                        'target_prices', backup_table_name)
        raise

    #Feature PECTEN-9
    try:
        after_insert(args.google_key_path,common_parameters["BQ_DATASET"],'target_prices',from_date,to_date,
                     storage_client)
    except AssertionError as e:
