...
#This plots the average twitter sentiment by week. 
avg_twitter_sent<-function(data_sent,constituent){
  if(constituent=='Adidas'){
    sent_df<- data_sent[data_sent$constituent==tolower(constituent),]}else{
      sent_df <- data_sent[data_sent$constituent==constituentt,]}
  title_str = paste('Weekly Twitter Sentiment for ',constituent, sep='')
